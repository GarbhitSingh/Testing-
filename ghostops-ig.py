...
from telegram import (
    Update, ReplyKeyboardMarkup, KeyboardButton, ReplyKeyboardRemove, constants
)
...
# ==============================================================================
# 💀 GHOSTOPS CONFIGURATION
# ==============================================================================
BOT_TOKEN = "YOUR_BOT_TOKEN_HERE"           # ⚠️ INSERT TOKEN
ADMIN_ID = 1234567890                        # ⚠️ INSERT ADMIN ID
DB_FILE = "ghostops_prime.db"

# ⚡ Performance Tuning
MAX_PROXY_FAILS = 3           # Tolerance before banning a proxy
PROXY_CHECK_TIMEOUT = 10      # Fast fail for dead proxies
REQUEST_TIMEOUT = 30          # Instagram timeout
MAX_CONCURRENT_CREATIONS = 5  # Simultaneous threads
MAX_PROXY_CHECKS = 20         # Parallel proxy validator count
...
async def get_working_proxy() -> Optional[str]:
    """Smart retrieval: Prioritizes active proxies with lowest fail count."""
    async with aiosqlite.connect(DB_FILE) as db:
...
class InstagramSession:
    def __init__(self, proxy: Optional[str] = None):
        self.proxy = proxy
        # 🛡️ TLS Fingerprint Masquerading (Chrome 110)
    ...
    async def fetch_initial_data(self):
        """Handshake with Instagram to generate valid session tokens."""
    ...
    async def step_2_verify_create(self, email: str, otp: str, context_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            if '"account_created":true' in r_create.text:
                ...
                logger.error(f"Creation Failed Dump: {r_create.text}")
                return {"status": False, "msg": "Instagram Rejected Final Payload"}
...
async def handle_email(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with creation_semaphore:
        ...
        if result['status']:
            ...
            if "Proxy" in result['msg'] or "Banned" in result['msg']:
...
async def handle_proxy_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    added = sum(1 for p, alive in results if alive and await add_valid_proxy(p) is None)

...
CHANNEL_IDS = ['@YourChannel']  # Add your channel usernames here, e.g. ['@channel1', '@channel2']
...
def handle_instagram_linking(chat_id, message_id=None, mode='connect'):
    ...
    for retry in range(1, max_retries + 1):
        ...
        try:
            ...
            ig_user = shein_json.get('user_data', {}).get('username', 'N/A')
            followers = shein_json.get('user_data', {}).get('followers_count', 'N/A')
            acc_type = shein_json.get('user_data', {}).get('account_type', 'N/A')
            
            voucher_data = shein_json.get('voucher', {})
            if not voucher_data.get('voucher_amount'):
                bot.send_message(chat_id, "⌛ Polling for voucher...")
                voucher_data = fetch_voucher_with_retry(creator_token, ip)
